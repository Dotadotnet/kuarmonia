...
postSchema.pre("save", async function (next) {
  ...
  if (!this.canonicalUrl) {
    this.canonicalUrl = `${defaultDomain}/post/${
      this.slug
...
postSchema.pre("save", async function (next) {
  ...
  if (this.isNew || this.isModified("visibility")) {
    this.metaRobots =
      this.visibility === "private" ? "noindex, nofollow" : "index, follow";
  }
    ...
    try {
      ...
      if (category) {
        ...
        if (combinedMetaTitle.length > 60) {
          ...
          combinedMetaTitle = `${this.title.substring(
            0,
            this.title.length - excessLength
          )} | ${category.title}`;
        }
        ...
        if (combinedMetaDescription.length > 160) {
          ...
          combinedMetaDescription = `${this.description.substring(
            0,
            this.description.length - excessLength
          )} | ${category.title}`;
        }
        ...
        this.metaDescription =
          this.description.length > 160
            ...
            : this.description;
        this.metaKeywords =
          this.metaKeywords.length > 0 ? this.metaKeywords.slice(0, 10) : [];
      }
    } catch (error) {
      console.error(
        "خطا در تنظیم metaTitle، metaDescription و metaKeywords:",
        error
      );
    }
  }
