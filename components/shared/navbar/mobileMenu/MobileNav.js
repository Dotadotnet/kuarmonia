...
const MobileNav = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 flex flex-col backdrop-blur-sm items-start justify-start w-screen h-screen overflow-y-hidden"
    >
      <OutsideClick onOutsideClick={() => setIsOpen(false)} className="relative w-full h-full">
        <div
          className="relative w-full h-full"
          onClick={(e) => e.stopPropagation()} // Prevents closing when clicking inside the menu
        >
          <div className="flex absolute w-2/3 h-2/3 items-center rounded-lg bg-white justify-start gap-10 flex-col top-1/2 right-5 transform pt-8 -translate-y-1/2">
            {[
